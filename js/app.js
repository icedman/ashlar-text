...
const EditorStatus = (props) => {
    const [ state, setState ] = React.useState({cursor: app.cursor(),language:app.language()});
    const updateCursor = () => {
        setState({
            ...state,
            cursor: app.cursor(),
    ...
    return <View id='status::editor' permanent={true} style={styles.bar}>
        <Text id='status::cursor' style={styles.cursor}>
            Line: {state.cursor[0]+1} Column: {state.cursor[1]+1}
        </Text>
        <Text id='status::language' style={styles.language}>{state.language}</Text>
